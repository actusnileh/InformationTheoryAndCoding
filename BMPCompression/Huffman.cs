...
namespace BMPCompression
{
    public class Huffman
    {
        ...
        public byte[] Compress(string input)
        {
            ...
            string jsonTree = JsonSerializer.Serialize(root);

            // Convert JSON string to byte array
            byte[] treeBytes = Encoding.UTF8.GetBytes(jsonTree);

            // Combine tree size and tree bytes
            byte[] compressedBytes = [.. BitConverter.GetBytes(treeBytes.Length), .. treeBytes, .. encodedBytes];

            return compressedBytes;
        }

        // Генерация таблицы частот символов
        private static Dictionary<char, int> GenerateFrequencies(string input)
        {
            if (string.IsNullOrEmpty(input))
                return [];
        ...
        public string Decompress(byte[] encodedBytes)
        {
            ...
            // Deserialize Huffman tree from JSON
            string jsonTree = Encoding.UTF8.GetString(treeBytes);
            HuffmanNode root = JsonSerializer.Deserialize<HuffmanNode>(jsonTree);
